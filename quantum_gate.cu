...
void display_inputs(float *U, float *a, int n, int N)
{
	printf("\n U : ");
	for(int i=0; i<4; i++)
		printf("%f ", U[i]);
...
void write_results(float *result, int N, char *FILENAME, float execution_time)
{
	FILE *FP = fopen(FILENAME, "w");
	for(int i=0; i<N; i++)
		fprintf(FP, "%.3f\n", result[i]);
       fprintf(FP, "Execution Time (ms) : %f", execution_time);
       fclose(FP);
}
...
int main(int argc, char *argv[])
{
	...
	// write result
	write_results(result, N, OP_FILENAME,  execute_time);
	for(int i=0; i<N; i++)
           printf("%.3f\n", result[i]);
